  ...
  const typeText = (text) => {
    ...
    const intervalId = setInterval(() => {
      if (currentIndex <= text.length) {
        ...
        replacedText = replacedText.replace(/\*/g, "&bull;");
        replacedText = replacedText.replace(listRegex, "$1<li>$2</li>");
        if (replacedText.includes("<li>")) {
          replacedText = `<ul>${replacedText}</ul>`;
        }
    ...
    <div className={`main ${darkMode ? "dark-mode" : ""}`}>
      <div className="nav">
        ...
        <svg
          ...
          width="30"
          height="30"
          fill={darkMode ? "#fff" : "rgba(0,0,0,0.7)"}
        >
          {darkMode ? (
        ...
        <div className="main-bottom">
          ...
          <div>
            <p className="bottom-text">
              DialogAi may display inaccurate info, including about people, so
              double-check its responses. Your privacy & DialogAi
            </p>
          </div>
        </div>
      </div>
